    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .logo {
            ...
            height: 48px;
            border-radius: 10px;
            background: linear-gradient(135deg, var(--accent), #6dd1ff);
            display: flex;
            align-items: center;
        ...
        .card {
            ...
            scrollbar-color: rgba(86,193,255,0.3) transparent;
        }
        ...
        .card::-webkit-scrollbar-thumb {
            background-color: rgba(86,193,255,0.3);
            border-radius: 8px;
            border: 2px solid transparent;
        }

        .card::-webkit-scrollbar-thumb:hover {
            background-color: rgba(86,193,255,0.5);
        }
        ...
        .primary {
            background: linear-gradient(90deg, var(--accent), #7ad9ff);
            padding: 10px 14px;
            border-radius: 10px;
            border: none;
            color: #012;
...
<body>
    ...
    <script>
        ...
        function render(items) {
            listEl.innerHTML = '';
            if(!items.length) { 
                countEl.textContent = 'Файлов не найдено.';
                return;
            }
            ...
            for(const f of items) {
                ...
                thumb.textContent = fileIcon(f.name);
                ...
                sub.className = 'sub';
                sub.textContent = `${humanSize(f.size)} • ${fmtDate(f.mtime)} • ${f.type||''}`;
                ...
                item1.href = '#';
                item1.className = 'dd-item';
                item1.textContent = 'Скопировать прямую ссылку';

                const item2 = document.createElement('a');
                item2.href = '#';
                item2.className = 'dd-item';
                item2.textContent = 'Скопировать команду для Linux (wget)';
                ...
                function copy(text) {
                    if(navigator.clipboard && navigator.clipboard.writeText) {
                        ...
                        t.select();
                        try { document.execCommand('copy'); showToast('Скопировано в буфер'); } 
                ...
                item2.addEventListener('click', (e) => { 
                    e.preventDefault(); 
                    const wget = `wget -O "${f.name.replace(/\"/g,'\\"')}" "${fileUrl}"`; 
                    copy(wget); 
